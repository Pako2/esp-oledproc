...
function espoledJsMinify (cb) {
    return pump([
        gulp.src('../../src/websrc/js/espoled.js'),
        uglify(),
        gulp.dest('../../src/websrc/gzipped/js/'),
    ] );
}

function espoledJsGz() {
    return gulp.src("../../src/websrc/gzipped/js/espoled.js")
        .pipe(gzip({
            append: true
        }))
...
function espoledJsGzh(cb) {
    var source = "../../src/websrc/gzipped/js/" + "espoled.js.gz";
    var destination = "../../src/webh/" + "espoled.js.gz.h";
    ...
    wstream.write('#define espoled_js_gz_len ' + data.length + '\n');
    wstream.write('const uint8_t espoled_js_gz[] PROGMEM = {')
...
async function runner() {
    const scriptTasks = gulp.series(espoledJsMinify, espoledJsGz, espoledJsGzh, scriptsConcat, scripts);
